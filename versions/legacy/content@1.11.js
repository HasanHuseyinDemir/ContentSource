...
contents.map(async(item)=>{
        switch(item.type){
            ...
            case "text":{let source = await fetch(item.src);let data = await source.text();
                if(item.test==true){
                    interpolationTest(data,item.name,"text")
                }else{
                    creaText(item.name,data);
                }
            };break;
            ...
            if(item.text==true||item.text=="true"){
                creaText(item.name,item.data()),
                item.render=()=>{
                    document.querySelectorAll(item.name).forEach(page=>page.textContent=item.data());
                    console.log("Merhaba");
                }
            }else{
...
interpolationTest=(str,component,condition)=>{
    ...
    if(isPolated(isStarter,isEndPoint)==false){
        switch(condition){
            ...
            case "text":creaText(component,str);break;
...
unmount=(page)=>{
    page.data="";
    let index = document.querySelectorAll(page.name);
    index.forEach((i,index)=>{
        let element = document.querySelectorAll(page.name)[index];
        element.innerHTML=page.data;
    })
    if(contents.includes(page)){
       contents.splice(contents.indexOf(page));
    }
}
...
fileToRender=async(els,sr)=>{
    let source = await fetch(sr);let data = await source.text();
    let index = document.querySelectorAll(els);
    index.forEach((i,index)=>{
    let elements = document.querySelectorAll(els)[index];
    renderTest(data,elements);
})};
