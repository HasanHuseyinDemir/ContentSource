...
test={
    ...
    test.customscript("defer",str);
    },
customscript:async(condition,str)=>{
    let valid=false;
...
set=(element)=>{
    ...
    document.body.querySelectorAll(".set").forEach((contentelem)=>{
            ...
            var forEac = contentelem.getAttribute("forEach");

            //sub for foreach
            let cond=contentelem.getAttribute("conditions");
            var returns=contentelem.getAttribute("returns");

            if(forEac&&returns&&cond){
                eval(forEac).forEach(()=>{
                    content.states.push({})
                })
            }
