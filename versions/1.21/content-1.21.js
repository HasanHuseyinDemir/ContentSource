...
content={
    ...
    dry:{
        ...
        dParser(str,element){
            ...
            dParse=(dParse.parseFromString(str,"text/html"));
            // element.appendChild(dParse.body.childNodes[0])
            let elementArray=new DocumentFragment();
            dParse.body.childNodes.forEach((elem,index)=>{
                elementArray.appendChild(elem)
            })
    ...
    statistics:{
        ...
        pageload:(time)=>{
            let date=new Date();
            switch(time){
                case "second":return Math.floor((date-content.statistics.now)/1000);
                case "minute":return (((date-content.statistics.now)/1000)/60);
                default:return date-content.statistics.now;
            }
        },
        latestRendered:[
...
test={
    ...
    //#Tamamlandı str ile replace edilmesi gerekiyor asıl {{ }}
...
set=(element)=>{
    ...
    document.body.querySelectorAll(element).forEach((contentelem)=>{
            contentelem.querySelectorAll(".set").forEach((contentelems)=>{
                ...
                if(data){
                    contentelems.innerHTML!=(eval(data).toString())?contentelems.innerHTML=(eval(data).toString()):"";
                }
    ...
    document.body.querySelectorAll(".set").forEach((contentelem)=>{
            ...
            if(data){
                contentelem.innerHTML!=(eval(data).toString())?contentelem.innerHTML=(eval(data).toString()):"";
            }
